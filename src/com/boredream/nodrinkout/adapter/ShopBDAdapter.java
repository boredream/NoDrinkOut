...
public class ShopBDAdapter extends BaseAdapter {

	private Context context;
	private List<CoffeeShopOfBD> datas;
	private ImageLoader imageLoader;

	public ShopBDAdapter(Context context, List<CoffeeShopOfBD> datas) {
	...
	@Override
	public CoffeeShopOfBD getItem(int position) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		// set data
		CoffeeShopOfBD item = getItem(position);
		imageLoader.displayImage(item.getImgUrl(), holder.iv_image, 
				ImageOptHelper.getShopOptions(DisplayUtils.dip2px(context, 2)));
		holder.tv_subhead.setText(item.name);
		holder.tv_body.setText(item.address);
